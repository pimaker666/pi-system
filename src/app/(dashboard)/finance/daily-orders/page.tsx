...
export default async function DailyOrdersPage({ searchParams }: {
  ...
  return <div className="space-y-6">
    ...
    <form className="grid gap-3 rounded-md border p-4 md:grid-cols-4 xl:grid-cols-8">
      ...
      <select name="salesperson" defaultValue={filters.salesperson ?? ''} className="h-10 rounded-md border bg-background px-3 text-sm"><option value="">全部业务员</option>{options.salespeople.map((person) => <option key={person.id} value={person.id}>{displayProfileName(person)}</option>)}</select>
      ...
      <div className="flex gap-2 xl:col-span-8"><Button type="submit">筛选</Button><Button asChild type="button" variant="outline"><Link href="/finance/daily-orders">清空</Link></Button><Button asChild type="button" variant="outline"><a download href={`/api/finance/daily-orders/export/xlsx${query ? `?${query}` : ''}`}><Download className="h-4 w-4" />XLSX</a></Button><Button asChild type="button" variant="outline"><a href={`/api/finance/daily-orders/export/pdf${query ? `?${query}` : ''}`}><FileText className="h-4 w-4" />PDF</a></Button></div>
    </form>
