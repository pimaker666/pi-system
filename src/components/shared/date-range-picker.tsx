...
function rangeFor(preset: Preset): DateRange | null {
  ...
  const date = parseDate(chinaToday())
        ...
        <div className="flex flex-col sm:flex-row">
          ...
          <div className="min-w-0 flex-1 p-3">
            <div className="mb-3 flex items-center justify-between">
              <Button type="button" variant="ghost" size="icon" onClick={() => setViewMonth(new Date(viewMonth.getFullYear(), viewMonth.getMonth() - 1, 1))}><ChevronLeft className="h-4 w-4" /></Button>
              <span className="text-sm font-medium">{viewMonth.getFullYear()}年 {viewMonth.getMonth() + 1}月</span>
              <Button type="button" variant="ghost" size="icon" onClick={() => setViewMonth(new Date(viewMonth.getFullYear(), viewMonth.getMonth() + 1, 1))}><ChevronRight className="h-4 w-4" /></Button>
            </div>
