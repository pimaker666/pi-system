...
export function DailyOrderWorkflowPanel({
  ...
  const canClaim = isWorkflowOwner && status === 'unclaimed'
  const canBind = (isWorkflowOwner || isFinance) && status !== 'unclaimed' && status !== 'approved'
  const canSubmit = isWorkflowOwner && (status === 'claimed' || status === 'rejected') && !!workflow.customer_id
  const canReviewPerformance = isFinance && status === 'submitted'
  const canCommission = isFinance && status === 'approved'
  const canRequestChange = isWorkflowOwner && status !== 'unclaimed'
