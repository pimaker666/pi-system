    ...
    <form className="space-y-6" onSubmit={submit}>
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          <div className="space-y-2"><Label>订单归属人</Label><Select value={salespersonId} onValueChange={setSalespersonId}><SelectTrigger><SelectValue placeholder="选择订单归属人" /></SelectTrigger><SelectContent>{assignedSalespeople.map((person) => <SelectItem key={person.id} value={person.id}>{displayProfileName(person)}</SelectItem>)}</SelectContent></Select></div>
