...
export function AccountLifecycleActions({
  ...
  function submitHandover() {
    startTransition(async () => {
      ...
      toast.success(
        summary
          ? `交接完成：客户 ${summary.customer_count} 个，未完成业务订单 ${summary.business_order_count} 个，店铺分配 ${summary.shop_assignment_count} 个`
          : '交接完成',
      )
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Select value={targetId} onValueChange={setTargetId}>
                <SelectTrigger>
                  <SelectValue placeholder="选择已通过审核的非财务账号" />
                </SelectTrigger>
                <SelectContent>
            ...
            <label className="flex items-start gap-3 text-sm">
              ...
              <span>
                同时转移这些客户的未完成业务订单
                <span className="block text-xs text-muted-foreground">
                  包含草稿、待审核、已驳回及已审核但未完成的订单；已完成或特殊关闭订单保持原归属。
                </span>
              </span>
            </label>
