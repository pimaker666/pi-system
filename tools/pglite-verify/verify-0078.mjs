...
try {
  ...
  await apply('0078_freight_commission_default_rate.sql')
  console.log('PASS 0078 is idempotent')

  const settings = await db.query(
    'select id, freight_commission_rate from public.finance_freight_commission_settings where id = 1',
  )
  assert(settings.rows.length === 1, 'default freight setting is seeded once')
  assert(Number(settings.rows[0].freight_commission_rate) === 0, 'default freight rate starts at zero')

  const constraint = await db.query(`
    select 1
    from pg_constraint
    where conname = 'finance_business_order_commission_cost_rate_valid'
  `)
  assert(constraint.rows.length === 1, 'zero-cost freight rate constraint exists')
} finally {
  await db.close()
}
