-- 0048: Allow adjust_business_order_items to recalculate derived daily totals
-- after payment/shipment, so approved orders can still receive appended items.
--
-- The order-level daily-field trigger is bypassed only when this trusted RPC is
-- running; manual edits through create/update_business_order_v4 keep the lock.
...
as $$
begin
  if current_setting('app.adjust_business_order_items_rpc', true) = 'true' then
    return new;
  end if;
