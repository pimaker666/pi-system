-- 0004_pi_item_image.sql
-- PI 明细新增 image_url（开单时快照产品图片），并让原子创建函数写入该字段。
-- 幂等：可安全重复执行。
...
comment on column public.pi_items.image_url is 'PI 开具时的产品图片快照 URL';

-- 2) 更新原子创建函数：插入明细时带上 image_url（函数签名不变，仍通过 p_items jsonb 传值）
...
begin
  ...
  loop
    insert into public.pi_items (
      pi_id, product_id, sku, name, description, image_url,
