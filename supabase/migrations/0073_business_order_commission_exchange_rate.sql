...
alter table public.finance_business_order_commissions
  add column if not exists settlement_exchange_rate_to_cny numeric(12, 6);

alter table public.finance_business_order_commissions
  drop constraint if exists finance_business_order_commission_exchange_rate_valid;
alter table public.finance_business_order_commissions
  add constraint finance_business_order_commission_exchange_rate_valid
  check (settlement_exchange_rate_to_cny is null or settlement_exchange_rate_to_cny > 0);

comment on column public.finance_business_order_commissions.freight_cost is
  '运费成本（人民币）。';
comment on column public.finance_business_order_commissions.settlement_exchange_rate_to_cny is
  '美元订单提成结算时使用的 USD→CNY 汇率。';
comment on table public.finance_business_order_profit_fees is
  '业务订单利润核算的订单级手续费；美元订单按提成结算汇率折算为人民币。';
  ...
  with eligible_orders as (
    ...
    select i.order_id,
      ...
      coalesce(sum(
        ...
        * case when bo.currency = 'USD' then coalesce(fc.settlement_exchange_rate_to_cny, 1) else 1 end
    ...
    select bo.id, eo.profit_period, bo.order_date, bo.order_number, bo.external_order_number,
      ...
      case when bo.currency = 'USD' then coalesce(fc.settlement_exchange_rate_to_cny, 1) else 1 end as exchange_rate,
      (coalesce(bo.total_sales_amount, 0) + coalesce(a.received_allocations, 0))
        * case when bo.currency = 'USD' then coalesce(fc.settlement_exchange_rate_to_cny, 1) else 1 end as received_amount,
      (coalesce(bo.total_shipping_received_amount, 0) + coalesce(a.shipping_allocations, 0))
        * case when bo.currency = 'USD' then coalesce(fc.settlement_exchange_rate_to_cny, 1) else 1 end as freight_received_amount,
      ...
      coalesce(fee.fee_amount, 0)
        * case when bo.currency = 'USD' then coalesce(fc.settlement_exchange_rate_to_cny, 1) else 1 end as fee_amount
